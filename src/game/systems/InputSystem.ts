...
export class InputSystem {
  ...
  private horizontalRepeatMs = 0;
  private softDropRepeatMs = 0;
  private leftWasDown = false;
  private rightWasDown = false;
  private softDropWasDown = false;

  private readonly handleRotateUp = () => this.handlers.rotate();
  private readonly handleRotateW = () => this.handlers.rotate();
  private readonly handleHardDrop = () => this.handlers.hardDrop();
  private readonly handleHoldShift = () => this.handlers.hold();
  private readonly handleHoldC = () => this.handlers.hold();
  ...
  update(delta: number): void {
    this.horizontalRepeatMs += delta;
    this.softDropRepeatMs += delta;
    ...
    if (leftDown && !this.leftWasDown) {
      this.horizontalRepeatMs = 0;
      this.handlers.moveLeft();
    } else if (leftDown && this.horizontalRepeatMs >= 110) {
      this.horizontalRepeatMs = 0;
      this.handlers.moveLeft();
    }
    ...
    } else if (rightDown && this.horizontalRepeatMs >= 110) {
      this.horizontalRepeatMs = 0;
      this.handlers.moveRight();
    }

    if (softDropDown && !this.softDropWasDown) {
      this.softDropRepeatMs = 0;
      this.handlers.softDrop();
    } else if (softDropDown && this.softDropRepeatMs >= 55) {
      this.softDropRepeatMs = 0;
      this.handlers.softDrop();
    }

    if (!leftDown && !rightDown) {
      this.horizontalRepeatMs = 110;
    }

    if (!softDropDown) {
      this.softDropRepeatMs = 55;
    }
